...
const getView = async (req: Request, res: Response) => {
  const players = await PlayerModel.find({ isCaptain: true }).populate('nation').exec();

  return res.render("captain/view", { players, user: req.user });
};

const addView = async (req: Request<RequestParams>, res: Response) => {
  const nation = await NationModel.find({});

  return res.render("players/add", { nation });
};
